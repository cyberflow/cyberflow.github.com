...
# override atribute
node.override['chef_server']['postgresql']['username'] = 'postgres'
node.override['chef_server']['postgresql']['data_dir'] = '/var/lib/postgresql/9.2/main'
...
pg_hba_config = File.join('/etc/postgresql/9.2/main/', "pg_hba.conf")
...
execute "/usr/bin/createdb -T template0 -E UTF-8 opscode_chef" do
  user node['chef_server']['postgresql']['username']
...
execute "migrate_database" do
  command "/usr/bin/psql opscode_chef < priv/pgsql_schema.sql"
  cwd chef_db_dir
  user node['chef_server']['postgresql']['username']
  action :nothing
end

execute "/usr/bin/psql -d 'opscode_chef' -c \"CREATE USER #{node['chef_server']['postgresql']['sql_user']} WITH SUPERUSER ENCRYPTED PASSWORD '#{node['chef_server']['postgresql']['sql_password']}'\"" do
  cwd chef_db_dir
  user node['chef_server']['postgresql']['username']
...
execute "grant opscode_chef privileges" do
  command "/usr/bin/psql -d 'opscode_chef' -c \"GRANT ALL PRIVILEGES ON DATABASE opscode_chef TO #{node['chef_server']['postgresql']['sql_user']}\""
  user node['chef_server']['postgresql']['username']
  action :nothing
end

execute "/usr/bin/psql -d 'opscode_chef' -c \"CREATE USER #{node['chef_server']['postgresql']['sql_ro_user']} WITH SUPERUSER ENCRYPTED PASSWORD '#{node['chef_server']['postgresql']['sql_ro_password']}'\"" do
  cwd chef_db_dir
  user node['chef_server']['postgresql']['username']
...
execute "grant opscode_chef_ro privileges" do
  command "/usr/bin/psql -d 'opscode_chef' -c \"GRANT ALL PRIVILEGES ON DATABASE opscode_chef TO #{node['chef_server']['postgresql']['sql_ro_user']}\""
  user node['chef_server']['postgresql']['username']
  action :nothing
end
